...
export default
class LeafDartAnimation implements AnimationStep
{
	...
	constructor(step:TurnStep, vm:ViewManager) 
	{
		...
		step.Events.filter((e) => e.event == "damage").forEach((e) => {
			...
			if (source && targetPuppet)
			{
				...
				this.anim.steps.push(new SimultaneousAnimation([
					new ProjectileAnimation({
						node: noteIcon,
						startPos: {x: () => source.position().x, y: () => source.position().y - noteIcon.getHeight() * 0.2 / 2},
