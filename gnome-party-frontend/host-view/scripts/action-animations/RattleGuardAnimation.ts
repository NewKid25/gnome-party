...
export default
class RattleGuardAnimation implements AnimationStep
{
	anim:AnimationStep
	play() 
	{
	...
	constructor(step:TurnStep, vm:ViewManager) 
	{
		...
		if (enemy)
		{
			blockIcon.position({x: enemy.position().x - enemy.width() * 0.8, y: enemy.position().y});
			this.anim =	new SimultaneousAnimation([
				new SpawnIconAnimation({
					node: blockIcon,
					position: {x: () => enemy.position().x, y: () => enemy.position().y - blockIcon.getHeight() * 0.2 / 2},
					layer: vm.uiLayer
				}),
