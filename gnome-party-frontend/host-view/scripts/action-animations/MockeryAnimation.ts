...
export default
class MockeryAnimation implements AnimationStep
{
	anim:AnimationStep
	play() 
	{
	...
	constructor(step:TurnStep, vm:ViewManager) 
	{
		...
		console.log(noteIcon.getSelfRect());
			...
			this.anim =	new SimultaneousAnimation([
				new AnimationSequence([
					new FunctionStep(() => {
						...
						noteIcon.position({
							x: player.position().x,
							y: player.position().y - noteIcon.getHeight() * 0.4
						});
					}),
					...
					new AnimationPause(500),
					new ProjectileAnimation({
						node: noteIcon,
						startPos: {x: () => player.position().x, y: () => {console.log(noteIcon.getHeight()); return player.position().y - noteIcon.getHeight() * 0.4}},
					...
					// new FunctionStep(() => console.log(noteIcon.getSelfRect())),
				])
			])
