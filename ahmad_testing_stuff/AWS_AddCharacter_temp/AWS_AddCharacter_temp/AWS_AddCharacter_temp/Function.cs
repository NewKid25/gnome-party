...
namespace AWS_AddCharacter_temp
{
    public class Function
    {
        ...
        private async Task<APIGatewayProxyResponse> HandleAddCharacter(APIGatewayProxyRequest request, ILambdaContext context)
        {
            ...
            var allowedCharacterTypes = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                "player",
                "enemy"
            };

            if (!allowedCharacterTypes.Contains(character.CharacterType))
            {
                return new APIGatewayProxyResponse
                {
                    StatusCode = 400,
                    Body = "CharacterType must be 'player' or 'enemy'"
                };
            }
            ...
            var item = new Dictionary<string, AttributeValue>
            {
                ...
                { "CharacterType", new AttributeValue { S = character.CharacterType } }
            };
